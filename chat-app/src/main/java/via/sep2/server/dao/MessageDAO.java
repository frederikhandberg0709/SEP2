...
public class MessageDAO {
    ...
    public MessageDTO saveMessage(MessageDTO message) throws SQLException {
        String sql = """
                INSERT INTO messages (room_id, direct_chat_id, sender_username, content, type, timestamp, reply_to_message_id)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                RETURNING id
                """;
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setObject(1, message.getRoomId() > 0 ? message.getRoomId() : null);
            stmt.setObject(2, message.getRoomId() <= 0 ? Math.abs(message.getRoomId()) : null);
            stmt.setString(3, message.getSenderUsername());
            stmt.setString(4, message.getContent());
            stmt.setTimestamp(6, new Timestamp(message.getTimestamp()));
            ...
            if (rs.next()) {
                ...
                if (message.getRoomId() <= 0) {
                    DirectChatDAO.getInstance().updateLastMessageTimestamp(Math.abs(message.getRoomId()));
                }
    ...
    public List<MessageDTO> getGroupChatMessages(int roomId, int limit) throws SQLException {
        String sql = """
                SELECT id, room_id, sender_username, content, type, timestamp, reply_to_message_id,
                       is_edited, edited_timestamp, is_deleted
                FROM messages
    ...
    public List<MessageDTO> getDirectChatMessages(int directChatId, int limit) throws SQLException {
        String sql = """
                SELECT id, direct_chat_id as room_id, sender_username, content, type, timestamp, reply_to_message_id,
                       is_edited, edited_timestamp, is_deleted
                FROM messages
        ...
        List<MessageDTO> messages = getMessagesFromQuery(sql, directChatId, limit);
        messages.forEach(msg -> msg.setRoomId(-msg.getRoomId()));
        return messages;
    }

    public List<MessageDTO> getMessagesAfter(int roomId, long timestamp, boolean isDirectChat) throws SQLException {
        String sql;
        if (isDirectChat) {
            sql = """
                    SELECT id, direct_chat_id as room_id, sender_username, content, type, timestamp, reply_to_message_id,
    ...
    public void editMessage(int messageId, String newContent, String editorUsername) throws SQLException {
    ...
    public void deleteMessage(int messageId, String deleterUsername) throws SQLException {
        if (!canUserDeleteMessage(messageId, deleterUsername)) {
            throw new SQLException("User not authorized to delete this message");
        }
    ...
    public MessageDTO getMessageById(int messageId) throws SQLException {
        String sql = """
                SELECT id, COALESCE(room_id, -direct_chat_id) as room_id, sender_username, content, type, timestamp,
                       reply_to_message_id, is_edited, edited_timestamp, is_deleted
    ...
    private boolean canUserEditMessage(int messageId, String username) throws SQLException {
        String sql = "SELECT sender_username FROM messages WHERE id = ? AND is_deleted = false";
    ...
    private boolean canUserDeleteMessage(int messageId, String username) throws SQLException {
        String sql = """
    ...
    private List<MessageDTO> getMessagesFromQuery(String sql, int id, int limit) throws SQLException {
        List<MessageDTO> messages = new ArrayList<>();
    ...
    private MessageDTO createMessageFromResultSet(ResultSet rs) throws SQLException {
